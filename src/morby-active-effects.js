...
import { applyDamage, handleResolvedSaveRequest, handleTurnEndEffects, handleTurnStartEffects } from "./effects.js";
...
Hooks.once("ready", () => {
    ...
    $(document).on('click', '.mae-save-success', async function () {
        ...
        if (halfDamage) {
            await applyDamage(combatant._id, formula, effectName, true);
        };
    ...
    $(document).on('click', '.mae-save-failure', async function () {
        ...
        // Apply effect damage
        await applyDamage(combatant._id, formula, effectName, false);
